...
void AVRController::Tick(float DeltaTime)
{
	...
	if (DirectionVector.X > 0)
	{
		...
		if (bIsGoingBackward)
		{
			...
			OnCommand.Broadcast(FString("backward stop"), 0);
		}

		OnCommand.Broadcast(FString("forward"), GetSpeedPercent(DirectionVector.X));
	}
	else
	{
		if (!bIsGoingBackward)
		...
		{
			bIsGoingBackward = false;
			OnCommand.Broadcast(FString("forward stop"), 0);
		}

		OnCommand.Broadcast(FString("backward"), GetSpeedPercent(-DirectionVector.X));
	}
	...
	if (DirectionVector.Y > 0)
	{
		...
		{
			bIsGoingBackward = false;
			OnCommand.Broadcast(FString("left stop"), 0);
		}

		OnCommand.Broadcast(FString("right"), GetSpeedPercent(DirectionVector.Y));
	...
	else
	{
		...
		{
			bIsGoingLeft = false;
			OnCommand.Broadcast(FString("right stop"), 0);
		}

		OnCommand.Broadcast(FString("left"), GetSpeedPercent(-DirectionVector.Y));
	}
	...
	if (DirectionVector.Z > 0)
	{
		...
		{
			bIsGoingDown = false;
			OnCommand.Broadcast(FString("down stop"), 0);
		}

		OnCommand.Broadcast(FString("up"), GetSpeedPercent(DirectionVector.Z));
	...
	else
	{
		...
		{
			bIsGoingUp = false;
			OnCommand.Broadcast(FString("up stop"), 0);
		}

		OnCommand.Broadcast(FString("down"), GetSpeedPercent(-DirectionVector.Z));
	}
}

float AVRController::GetSpeedPercent(float Value)
{
	return FMath::GetMappedRangeValueClamped(FVector2D(10, 20), FVector2D(0, 1), Value);
}
